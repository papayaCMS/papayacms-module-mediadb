...
* @version $Id: content_upload.php 39732 2014-04-08 15:34:45Z weinert $
*/
  ...
  function getParsedData() {
    ...
    if ($this->validSurfer) {
      ...
      if (!empty($_GET['id'])) {
        ...
        $mediaDBRpc = new base_mediadb_rpc;
        $mediaDBRpc->executeUploadProgressRPC($_GET['id']);
        ...
        if ($this->validSurfer) {
          ...
          if ($fileId) {
            ...
            if ((isset($this->params['title']) && $this->params['title'] != '') ||
              ...
              $this->mediaDB->databaseInsertRecord(
    ...
    if ($this->data['show_surfers_upload_newfile'] > 0) {
      ...
      if ($this->getFileCount() <= $this->data['maximum_uploaded_files']) {
        ...
        $result .= sprintf(
          ...
          papaya_strings::escapeHTMLChars($this->data['caption_upload_button'])
        );
        $result .= '</uploaddialog>';
      } else {
